...
const CAT: Record<Location["category"], { label: string; color: string; emoji: string }> = {
...
export function LocationCard({ location, variant = "default", className }: Props) {
  ...
  if (variant === "featured") {
    return (
      <div onClick={go} className={cn("relative h-64 rounded-2xl overflow-hidden cursor-pointer group shrink-0", className)}>
        <ImgWithFallback src={location.img} alt={location.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" />
        <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent" />
        <span className={cn("absolute top-3 left-3 text-[10px] font-semibold px-2 py-1 rounded-full border backdrop-blur-sm", cat.color)}>{cat.emoji} {cat.label}</span>
        <button onClick={bookmark} className={cn("absolute top-3 right-3 w-8 h-8 rounded-full flex items-center justify-center backdrop-blur-md transition-all active:scale-90", inPlan ? "bg-teal-500 text-white" : "bg-black/40 text-white hover:bg-teal-500/70")}>
  ...
  return (
    ...
    >
      <div className="relative h-48 overflow-hidden">
        <ImgWithFallback src={location.img} alt={location.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
        ...
        <button onClick={bookmark} className={cn("absolute bottom-3 right-3 w-8 h-8 rounded-full flex items-center justify-center backdrop-blur-md transition-all active:scale-90", inPlan ? "bg-teal-500 text-white" : "bg-black/50 text-white/80 hover:bg-teal-500/80")}>
