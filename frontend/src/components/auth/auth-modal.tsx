    ...
    <div className="space-y-1">
      ...
      <div className="relative">
        <input
          ...
          className={cn(
            "w-full px-3 py-2.5 rounded-xl border bg-muted text-foreground text-sm",
          ...
          <div key={label} className="flex items-center gap-1 flex-1">
            <div
              className={cn(
                ...
                i + 1 < step ? "bg-teal-500" :
                i + 1 === step ? "bg-teal-400" : "bg-border"
              )}
            />
          </div>
        ))}
              ...
              <button
                ...
                className={cn(
                  ...
                  selectedLang === l.code
                    ...
                    : "bg-muted border-border text-foreground hover:border-teal-500/20"
                )}
              >
      ...
      {step === 2 && (
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <Input label="Ism"      placeholder="Ali"      error={errors.name?.message}    {...register("name")} />
            <Input label="Familiya" placeholder="Valiyev"  error={errors.surname?.message} {...register("surname")} />
          </div>
          ...
          <Input label="Mamlakat (ixtiyoriy)" placeholder="O'zbekiston" {...register("country")} />
        ...
        <div className="text-center py-4 space-y-4 animate-fade-up">
          ...
          <div className="space-y-1.5 text-left">
            {[
              ...
              <p key={t} className="text-sm text-foreground/80">{t}</p>
            ))}
          </div>
          <button
            onClick={onClose}
...
export function AuthModal() {
  ...
  function handleClose() {
    closeAuthModal();
    // Reset tab after close animation
    ...
    <Dialog.Root open={authModalOpen} onOpenChange={(open) => !open && handleClose()}>
      <Dialog.Portal>
        ...
        >
          ...
          {/* Tabs */}
          <div className="flex rounded-xl bg-muted p-1 mb-5 gap-1">
